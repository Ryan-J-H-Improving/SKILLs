...
### Figure Asset Locations

- Save new crops and generated teaching aids used by the central `notes/study-notes.md` to the course workspace's `extracted/figures/` directory by default.
- A detailed weekly note linked from the central `Contents` may keep note-owned images in a sibling `assets/` or `figures/` directory inside the same course project. Record the exact relative path in `notes/figure-notes.md`.
- Do not create another storage convention without a concrete ownership need. Do not place note assets outside the course project.
- Do not move an existing validated image merely to normalize its directory name. Preserve working links and hashes unless relocation solves a real ownership or delivery problem and passes the full three-check release process again.
...
Use `pending_insertion` only when validation succeeded but the learner-facing note has not yet received the image. Use `reference_only` with a reason when a valid figure is intentionally not delivered. Never leave delivery status unstated.
